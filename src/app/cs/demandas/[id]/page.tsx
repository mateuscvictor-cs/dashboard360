...
export default function CSDemandaPage({
  params,
}: {
  ...
  useEffect(() => {
    fetch(`/api/demands/${id}`)
      .then((res) => {
        if (!res.ok) {
          if (res.status === 404 || res.status === 403) router.push("/cs/tarefas");
          return null;
        }
        return res.json();
      })
      .then(setDemand)
      .catch(() => router.push("/cs/tarefas"))
            ...
            <CardContent className="space-y-4">
              ...
              {demand.company && (
                <div>
                  ...
                  <Link
                    href={`/cs/empresas/${demand.company.id}`}
              ...
              {demand.sourceBooking && (
                <div>
                  ...
                  <Link
                    href={`/cs/agenda/${demand.sourceBooking.id}`}
                    className="flex items-center gap-2 text-sm font-medium hover:underline"
                  >
                    <FileText className="h-4 w-4" />
