...
export function Header({ title, subtitle, showFilters = true, action }: HeaderProps) {
  return (
    <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur-xl">
      <div className="flex h-16 flex-wrap items-center justify-between gap-2 px-4 md:px-6">
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <div className="relative group hidden md:block">
            ...
            <input
              ...
              className="h-10 w-72 max-w-[18rem] rounded-xl border bg-muted/50 pl-10 pr-12 text-sm outline-none transition-all duration-200 focus:bg-background focus:ring-2 focus:ring-primary/20 focus:border-primary/50"
            />
          ...
          {showFilters && (
            <Button variant="outline" size="default" className="gap-2 rounded-xl">
              ...
              <Badge variant="secondary" size="sm" className="ml-1 hidden sm:inline-flex">3</Badge>
            </Button>
          )}

          <Button variant="ghost" size="icon" className="relative shrink-0 rounded-xl group">
