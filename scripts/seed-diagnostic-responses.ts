...
const TASKS = [
  ...
  "Atualizar cadastros no sistema",
  "Conferir notas fiscais",
  "Responder emails de clientes",
  "Fazer follow-up de vendas",
...
const FRUSTRATIONS = [
  "Muita burocracia para aprovar coisas simples",
  "Sistemas que não conversam entre si",
  "Ter que fazer a mesma coisa várias vezes",
  "Falta de padrão nos processos",
  "Depender de outras áreas para tudo",
  "Retrabalho constante por erros de comunicação",
  "Perder tempo procurando informações",
  "Planilhas desatualizadas",
  "Muitos emails para resolver problemas simples",
...
async function seedResponses() {
  ...
  console.log("Iniciando seed de 35 respostas...\n");
  
  for (let i = 0; i < 35; i++) {
    const response = generateResponse(i);
    ...
    try {
      ...
      if (res.ok) {
        console.log(`✓ Resposta ${i + 1}/35: ${response.fullName} (${response.area})`);
      } else {
        const error = await res.json();
        console.log(`✗ Resposta ${i + 1}/35: ${error.error}`);
      }
    } catch (error) {
      console.log(`✗ Resposta ${i + 1}/35: Erro de conexão`);
    }
